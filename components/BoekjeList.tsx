...
export default function BoekjeList({ boekjes, onArchive, onUpdate, archiveMode }: {
  boekjes: any[],
  ...
  return (
    <div>
      {!archiveMode && (
        <Link href="/archief" className="text-sm text-gray-500 underline inline-block mb-4">
            ...
            {editId === b.id ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => saveEdit(b.id)} className="bg-green-600 text-white p-2">Opslaan</button>
                  <button onClick={cancelEdit} className="bg-gray-300 p-2">Annuleren</button>
                </div>
              </div>
            ) : (
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2">
                  {!archiveMode && (
                    <>
                      <button onClick={() => startEdit(b)} className="text-sm text-blue-600">Wijzig</button>
                      <button onClick={() => onArchive(b.id)} className="text-sm text-red-600">Archiveer</button>
                    </>
                  )}
                  {archiveMode && (
                    <button onClick={() => onArchive(b.id)} className="text-sm text-green-600">Herstel</button>
                  )}
                </div>
              </div>
            )}
